...
function epochToJsDate(epochTime){
  ...
  const spoElement = document.getElementById("spo");
    ...
    if (user) {
      ...
      chartRef.on('value', snapshot =>{
        ...
        dbRef.orderByKey().limitToLast(chartRange).on('child_added', snapshot =>{
          ...
          var respirationrate = jsonData.respirationrate;
          var spo2 = jsonData.spo2;
          var systolik = jsonData.systolik;
          var diastolik = jsonData.diastolik;
          var timestamp = jsonData.timestamp;
          // Plot the values on the charts
          plotValues(chartHR, timestamp, heartrate);
          plotValues(chartRR, timestamp, respirationrate);
          plotValues(chartSPO, timestamp, spo2);
          plotValues(chartSYS, timestamp, systolik);
          plotValues(chartDIA, timestamp, diastolik);
        });
      });
      ...
      dbRef.orderByKey().limitToLast(1).on('child_added', snapshot =>{
        ...
        var timestamp = jsonData.timestamp;
        // Update DOM elements
        hrElement.innerHTML = heartrate;
        rrElement.innerHTML = respirationrate;
        spoElement.innerHTML = spo2;
        sysElement.innerHTML = systolik;
        diaElement.innerHTML = diastolik;
      ...
      function createTable(){
        ...
        dbRef.orderByKey().limitToLast(100).on('child_added', function(snapshot) {
          if (snapshot.exists()) {
            ...
            content += '<td>' + respirationrate + '</td>';
            content += '<td>' + spo2 + '</td>';
            content += '<td>' + systolik + '</td>';
            content += '<td>' + diastolik + '</td>';
            content += '</tr>';
          ...
          if (snapshot.exists()) {
            ...
            reversedList.forEach(element =>{
              if (firstTime){ // ignore first reading (it's already on the table from the previous query)
                ...
                var respirationrate = element.respirationrate;
                var spo2 = element.spo2;
                var systolik = element.systolik;
                var diastolik = element.diastolik;
